...
export abstract class Model {
  ...
  async save(): Promise<this> {
    ...
    if (this._id || this.id) { // Assuming common PK names _id (Mongo) or id (SQL)
  ...
  static async create<T extends Model>(this: new (data?: Partial<T>) => T, data: Partial<T>): Promise<T> {
    if (!ormInstance) throw new Error('ORM instance not set on Model.');
  ...
  static async findById<T extends Model>(this: new () => T, id: string | number | ObjectId): Promise<T | null> {
    if (!ormInstance) throw new Error('ORM instance not set on Model.');
    ...
    if (!result && id instanceof ObjectId || typeof id === 'string') { // Check for MongoDB ObjectId specifically if first attempt fails
        result = await ormInstance.findOne(Ctor, { _id: id });
    }
     return result ? new Ctor(result) as T : null;
  }

  static async find<T extends Model>(this: new (data?: Partial<T>) => T, criteria: any): Promise<T[]> {
    if (!ormInstance) throw new Error('ORM instance not set on Model.');
    const Ctor = this as typeof Model;
  ...
  static async findOne<T extends Model>(this: new (data?: Partial<T>) => T, criteria: any): Promise<T | null> {
    if (!ormInstance) throw new Error('ORM instance not set on Model.');
    const Ctor = this as typeof Model;
  ...
  static async update<T extends Model>(this: new () => T, criteria: any, data: Partial<T>): Promise<number> {
    if (!ormInstance) throw new Error('ORM instance not set on Model.');
    const Ctor = this as typeof Model;
