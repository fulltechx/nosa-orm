...
export class RedisAdapter implements DatabaseAdapter {
    ...
    private _getKey(prefix: string, id: string): string {
        return `${prefix}:${id}`;
    }
    ...
    async insert(keyPrefix: string, data: any): Promise<any> {
        ...
        try {
            ...
            console.error(`Error inserting data for key ${key}:`, error);
            throw error;
        }
    }
    ...
    async findOne(keyPrefix: string, criteria: any): Promise<any | null> {
        ...
        if (!criteria || typeof criteria.id === "undefined")
            throw new Error('findOne requires "id" in criteria.');
        const key = this._getKey(keyPrefix, criteria.id);
        try {
            const result = await this.client.get(key);
            return result ? JSON.parse(result) : null;
        } catch (error) {
            console.error(`Error finding data for key ${key}:`, error);
            throw error;
        }
    }
            ...
            if (criteria && Object.keys(criteria).length > 0) {
                return objects.filter((obj) =>
                    Object.entries(criteria).every(
                        ([k, v]) => obj.hasOwnProperty(k) && obj[k] === v
                    )
                );
            }
            return objects;
        } catch (error) {
            console.error(`Error finding data for prefix ${keyPrefix}:`, error);
            throw error;
        }
    }
    ...
    async update(keyPrefix: string, criteria: any, data: any): Promise<number> {
        ...
        try {
            ...
            if (exists === 0) {
                // client.exists returns number of keys that exist
    ...
    async query(queryString: string, params?: any[]): Promise<any[]> {
        ...
        console.warn(
            'The "query" method in RedisAdapter is intended for raw Redis commands and is not fully implemented for general purpose querying like SQL adapters.'
        );
        throw new Error(
            "Method not implemented or not applicable for general queries in RedisAdapter."
        );
    }
}
